...
public class keyboardControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (CubeState.started && !CubeState.keyMove && !CubeState.autoRotateDrag && !CubeState.drag && undoMoveList.Count > 0 && Input.GetKeyDown(KeyCode.LeftArrow))
        {
            ...
            arrowGenerator.DeactivateArrow();
            //kociembaSolveList.Add(ReverseStep(previousMoveList[0]));
            // after reverting cube state the reverted move need to be performed again to get solved state so store it
